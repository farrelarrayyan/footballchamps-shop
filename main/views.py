...
def add_product_entry_ajax(request):
    ...
    is_popular = request.POST.get("is_popular") == "on"  # checkbox handling
...
def delete_product_entry_ajax(request, product_id):
    try:
        # hanya boleh hapus produk miliknya sendiri
        product = Product.objects.get(pk=product_id, user=request.user)
        product.delete()
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Account created! You are now officially a champ 🔥')
